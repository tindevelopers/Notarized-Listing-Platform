...
class ErrorMonitor {
  ...
  logError(log: Omit<ErrorLog, 'timestamp'>) {
    ...
    if (process.env.NODE_ENV === 'development' && log.suppressed && log.reason === 'ResizeObserver') {
      console.debug(`🔇 [${new Date(entry.timestamp).toTimeString()}] Suppressed ${log.reason}:`, log.message);
    }
  }
...
if (typeof window !== 'undefined' && process.env.NODE_ENV === 'development') {
  ...
  (window as any).getResizeObserverErrors = () => errorMonitor.getErrorsByReason('ResizeObserver');
  (window as any).clearErrorLogs = () => errorMonitor.clearLogs();
  
  console.log('🔍 Error monitoring enabled. Use printErrorStats(), getResizeObserverErrors(), clearErrorLogs() in console');
}
